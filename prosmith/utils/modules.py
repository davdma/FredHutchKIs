...
class MM_TNConfig(object):
    ...
    def __init__(
        self,
        s_hidden_size=767,
        p_hidden_size=1280, # can consider increasing protein embedding size by using 5000+ embedding of ESM2
        max_seq_len=1276,
        task='regression',
        num_classes=1
    ):
...
class Bert(nn.Module):
    def __init__(self, num_classes, emb_dim, no_layers, task):
...
class MM_TN(nn.Module):
    def __init__(self, config):
        ...
        self.main_bert = Bert(num_classes=config.num_classes,
